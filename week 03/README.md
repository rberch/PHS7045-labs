...
myfun <- function(...){
  pars <- list(...)
  lapply(pars, str)
  lapply(pars,data.table::address)
}

myfun(a=c(2:4),b=rep("boys",2))
```
...
herfun <- function(y,X){
  linmod <- lm(y~X)
  assign("mod_results", linmod,envir = .GlobalEnv)
}

herfun(y=c(2,4,1,5,6,8,2),X=c(-1,4,9,6,0,5,10))
```
...
metd <- fread("https://raw.githubusercontent.com/USCbiostats/data-science-data/master/02_met/met_all.gz")
# Download the data
...
head(the2data[, list(USAFID, WBAN, STATE)], n = 4)
```
